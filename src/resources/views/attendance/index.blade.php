...
<div class="attendance-form"> 
    <h1>| 勤怠一覧</h1>
      ...
      {{-- 前月ボタン --}}
       <a href="{{ route('attendance.index', ['date'  => \Carbon\Carbon::parse($date)->subMonth()->format('Y-m')]) }}" class="link-btn">
        ← 前月
        </a>
       ...
       <span class="month-label">{{ $date->format('Y/m') }}</span>
        ...
        {{-- 翌月ボタン --}}
         <a href="{{ route('attendance.index', ['date' => \Carbon\Carbon::parse($date)->addMonth()->format('Y-m')]) }}" class="link-btn">
        翌月 →
         </a>
        </div>
